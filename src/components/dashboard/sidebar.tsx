    ...
    <div className="h-screen w-64 bg-blue-800 text-white flex flex-col">
      ...
      <nav className="flex-1 p-4">
        <ul className="space-y-2">
          <li>
            <Link href="/dashboard" 
              className={`block p-2 rounded hover:bg-blue-700 transition ${isActive('/dashboard')}`}>
              Dashboard
            </Link>
          </li>
          ...
          {hasPermission('view_profile') && (
            <li>
              <Link href="/dashboard/profile" 
                className={`block p-2 rounded hover:bg-blue-700 transition ${isActive('/dashboard/profile')}`}>
                Meu Perfil
              </Link>
            </li>
          )}
          ...
          {hasPermission('view_users') && (
            <li>
              <Link href="/dashboard/users" 
                className={`block p-2 rounded hover:bg-blue-700 transition ${isActive('/dashboard/users')}`}>
                Usuários
              </Link>
            </li>
          )}
          ...
          {hasPermission('view_settings') && (
            <li>
              <Link href="/dashboard/settings" 
                className={`block p-2 rounded hover:bg-blue-700 transition ${isActive('/dashboard/settings')}`}>
                Configurações
              </Link>
            </li>
          )}
